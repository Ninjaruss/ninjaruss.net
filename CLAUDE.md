...
- **Journal tile** (`.journal-tile`, 4×2, core, gold highlight): the single dominant entry point, headed "Notes & Showcases". Root is a `div` (not `<a>` — avoids nested anchors); contains a header link to `/journal`, five real deep-link rows with right-aligned muted short dates (`.journal-tile__date`, no type chips), and an emblem strip of recent showcase entries.
- **Core tiles** (`.bento-tile--core`): Journal and Shelf (Media Log) with elevated gold glow and larger typography
- **Signal tiles**: Current activity indicators (Now, Latest) — Now shows the latest now-entry's title; Latest is 2×2 with a stripped-markdown excerpt and cycles client-side through the latest 2 notes + latest 1 showcase (interleaved note/showcase/note, 7s interval, 250ms fade via `.is-cycling`; static first entry under `prefers-reduced-motion`)
...
6. **Latest Tile**: Homepage 2×2 tile with "X days ago" indicator and excerpt; cycles client-side through the latest 2 notes and latest 1 showcase (interleaved note/showcase/note, 7s interval with fade swap; static under `prefers-reduced-motion`)
