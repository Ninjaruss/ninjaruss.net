...
This file provides guidance for Codex (Codex.ai/code) when working with this codebase.
...
- **notes**: uses sharedSchema without extensions (no tags; relatedness via `collections`), plus an optional `substackUrl`. The collection no longer produces pages; it is the slug→Substack redirect map (see the Substack section below)
...
- `/showcase` — `SplitViewLayout`, `section="showcase"`, showcase entries only, sorted newest first. **Bare** (`showSearch={false}`): no search, no type control, no draw deck, no RSS icon. Removing the draw deck means `/showcase` now auto-opens its newest entry on desktop on load — the deck used to be the only thing suppressing SplitViewLayout's ordinary auto-open behavior. Dissolved from the old `/journal`, which merged notes + showcases into one date-sorted list; notes moved to Substack (see the Substack section below), so the merge had nothing left to merge. (The old featured strip linking `/novel` and `/status` (then `/stream`) was removed earlier; those live in the NavPill now.)
- `/notes/[...slug]` — **Not a page.** Server-rendered (`export const prerender = false`) redirect: reads the `notes` collection, resolves the slug via `resolveNoteRedirect()`, and 301s to that note's `substackUrl`, or to the Substack archive if not backfilled/unknown. The rest parameter also matches bare `/notes`. See the Substack section below.
...
- `/journal` → redirects to `/showcase` (the merge had nothing left to merge once notes moved to Substack; `astro.config.mjs` `redirects`).
- `/notes/*` (including bare `/notes`) → server-rendered 301 to the note's Substack post, falling back to the Substack archive (`src/pages/notes/[...slug].astro`, not `astro.config.mjs`/`vercel.json`; see the Substack section below).
- `/rss.xml` → server-side 301 to `https://ninjaruss.substack.com/feed` (`vercel.json`, not `astro.config.mjs` — feed readers need a true 301). **`vercel.json` redirects do not apply under `astro dev`.**
...
locally-generated RSS feed of note content. Substack's own `/archive` already
lists everything, so a hand-synced copy would be strictly worse (`/journal`
now 301s to `/showcase`).

`src/content/notes/` stays in the repo — public via GitHub — as the redirect
map, not a source of pages. The schema gained one optional field,
`substackUrl` (optional so a half-written entry can't fail the build).
`/notes/[...slug].astro` is server-rendered (`prerender = false`) rather than
a static page — a prerendered `Astro.redirect` degrades to a meta-refresh
document, which feed readers ignore and search engines discount. It resolves
the target via `resolveNoteRedirect()` (`src/utils/noteRedirect.ts`): a
note's own `substackUrl`, or `https://ninjaruss.substack.com/archive` as the
fallback for a note not yet backfilled or an unknown slug.

**Backfill is ongoing manual work.** All 24 pre-migration notes need to be
posted to Substack and have `substackUrl` filled in by hand; until then their
slugs redirect to the archive. Two inbound prose links still point at
`/notes/*` and keep working through the redirect meanwhile:
`src/content/profile/about.md` → `/notes/i-am-ninjaruss` and
`src/content/now/2026-03.md` → `/notes/addiction`. Retarget both once those
notes are backfilled.

`/rss.xml` redirects to the Substack feed via `vercel.json` (not
`astro.config.mjs` — external destination, feed readers need a true 301).
**`vercel.json` redirects do not apply under `astro dev`** — verify against
the real deployment.

**The homepage still advertises the writing**, via a build-time feed read in
`src/utils/substack.ts`: `fetchSubstackPosts(limit)` fetches the Substack
feed and `parseSubstackFeed()` (hand-rolled, pure, no new XML dependency)
extracts title/link/pubDate/description. Both wrap every failure in
`try`/`catch` and return `[]` rather than throw — a Substack outage or
offline build must degrade the homepage tiles to showcase-only, never fail
`npm run build`. `decodeEntities()` in the same file decodes decimal and hex
numeric entities plus common named ones, with `&amp;` deliberately decoded
**last** to avoid double-decoding — a real bug caught during implementation.

Showcases stay canonical on the site. No cross-posting automation in either
direction; publishing to Substack is manual.
...
The `splitView/` directory is modular: `contentLoader`, `emblemAnimation`, `eventBindings`, `filterEngine`, `filterUI`, `idleManager`, `mediaHandlers`, `proseImageTilt`, `types`, `urlState`. (`drawCard` — the notes-only draw pool — was deleted with the draw-a-card deck in the Substack migration.)
...
- **Notes**: Philosophical fragments and thoughts. **No longer rendered on-site** — the site is the slug→Substack redirect map only; publish the prose on Substack and fill in `substackUrl` (see the Substack section above)
